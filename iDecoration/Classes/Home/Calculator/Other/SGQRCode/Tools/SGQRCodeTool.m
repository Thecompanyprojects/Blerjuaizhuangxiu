...
+ (UIImage *)createNonInterpolatedUIImageFormCIImage:(CIImage *)image withSize:(CGFloat)size {
    ...
    UIImage *imageHH =[UIImage imageWithCGImage:scaledImage];
...
+ (UIImage *)SG_generateWithLogoQRCodeData:(NSString *)data logoImageName:(UIImage *)logoImageName logoScaleToSuperView:(CGFloat)logoScaleToSuperView {
    ...
    // 把图片设置成正方形
//    CGFloat width = logoImageName.size.width > logoImageName.size.height ? logoImageName.size.height : logoImageName.size.width;
//    icon_image = [NSObject getSubImage:icon_image mCGRect:CGRectMake(0, 0, width, width) centerBool:YES];
