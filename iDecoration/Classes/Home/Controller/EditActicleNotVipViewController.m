//
//  EditActicleNotVipViewController.m
//  iDecoration
//
...
@interface EditActicleNotVipViewController ()<UITableViewDelegate,UITableViewDataSource,DesignCaseListMidCellDelegate,DesignCaseListHeadCellDelegate,UIGestureRecognizerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,SSPopupDelegate,ASProgressPopUpViewDataSource,UIActionSheetDelegate,VrCommenCellDelegate,VoteCommenCellDelegate,RedCommenCellDelegate>{
    ...
    BOOL isHaveSignUp; //no:没有活动报名 yes：有活动报名
    
    BOOL isHaveRed; //no:没有红包 yes:有红包
    
    CGFloat bottomCellH;
    
    CGFloat cellFourH;//红包的cellH
    
    NSIndexPath *addPath;// 添加图片到第几个cell前面
    NSIndexPath *modifyPath;// 修改的是第几个cell的图片
    NSInteger addVidelTag;//添加视频到第几个前面 (-1:放到最后一个)
    NSInteger deleteRedTag;//删除第几个红包
...
@implementation EditActicleNotVipViewController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        ...
        cell.delegate = self;
        [cell configCrib:self.voteDescribe isHaveVote:isHaveVote];
        return cell;
    }
    else if (indexPath.section==4) {
        RedCommenCell *cell = [RedCommenCell cellWithTableView:tableView indexpath:indexPath];
        cell.delegate = self;
        [cell configArray:self.redArray isHaveRed:isHaveRed];
        cellFourH = cell.cellH;
        return cell;
    }
    else{
        ActivityCommenCell *cell = [ActivityCommenCell cellWithTableView:tableView indexpath:indexPath];
        //        cell.delegate = self;
        [cell configWith:self.actStartTimeStr actEndTime:self.actEndTimeStr haveSignUp:self.haveSignUpStr signUpNum:self.signUpNumStr isHaveActivity:isHaveSignUp];
        if (!isHaveSignUp&&!self.isFistr) {
...
-(void)addMusic{
    //    SelectBgmController *vc = [[SelectBgmController alloc]init];
    //    [self.navigationController pushViewController:vc animated:YES];
...
-(void)addToActivity{
    ...
    vc.activityType = ActivityTypeCompany;
    vc.costName = self.costName;
    vc.cost = self.cost;
    ...
    vc.dictBlock = ^(NSMutableDictionary *dict) {
        ...
        self.costName = [dict objectForKey:@"costName"];
        isHaveSignUp = YES;
...
-(void)SignUpGes:(UITapGestureRecognizer *)ges{
    bottomCellIsHidden = YES;
    [hiddenStateDict removeAllObjects];
    NSInteger count = self.dataArray.count;
    for (int i = 0; i<count; i++) {
...
-(void)activitySignUpBtnClick:(UIButton *)btn{
    bottomCellIsHidden = YES;
    [hiddenStateDict removeAllObjects];
    NSInteger count = self.dataArray.count;
    for (int i = 0; i<count; i++) {
...
-(void)addToRed{
    ...
    if (!self.companyId) {
        [[PublicTool defaultTool] publicToolsHUDStr:@"公司不存在" controller:self sleep:1.5];
        return;
    }
    ZCHSelectCouponController *vc = [[ZCHSelectCouponController alloc]init];
    vc.companyId = [NSString stringWithFormat:@"%ld",self.companyId];
    [vc.haveSelectArray addObjectsFromArray:self.redArray];
    vc.backBlock = ^(NSMutableArray *array) {
        isHaveRed = YES;
        YSNLog(@"%@",array);
        [self.redArray removeAllObjects];
        [self.redArray addObjectsFromArray:array];
...
-(void)addRedPacket:(UITapGestureRecognizer *)ges{
...
-(void)deleteRedWith:(NSInteger)tag{
    deleteRedTag = tag;
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"是否确认删除红包？"
                                                        message:@""
                                                       delegate:self
                                              cancelButtonTitle:@"取消"
                                              otherButtonTitles:@"确定", nil];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (alertView.tag==200) {
        if (buttonIndex==1) {
            ...
            if (isFromSetWater){
                for (UIViewController *vc in self.navigationController.childViewControllers) {
                    if ([vc isKindOfClass:[ZCHNewsActivityController class]]) {
    ...
    if (alertView.tag==400) {
        if (buttonIndex==1) {
            ...
            [self.redArray removeObjectAtIndex:deleteRedTag];
...
-(void)successBtnClick:(UIButton *)btn{
    ...
    //    if (!isHaveSignUp) {
    //        [[PublicTool defaultTool] publicToolsHUDStr:@"请填写报名活动" controller:self sleep:1.5];
    //        return;
    //    }
...
-(void)pushData{
    ...
    if (self.coverImgUrl == nil || self.coverImgUrl.length == 0) {
        [[UIApplication sharedApplication].keyWindow hudShowWithText:@"请编辑封面"];
        return;
    }
    ...
    if (self.isFistr) {
        ...
        [designDict setObject:@(self.companyId) forKey:@"constructionId"];
        ...
        if (isHaveSignUp) {
            ...
            [activityDict setObject:self.cost forKey:@"money"];
            [activityDict setObject:self.costName forKey:@"costName"];
        }
        ...
        NSInteger isLeader = 0;
        if (self.jobTag==1002||self.jobTag==1003||self.jobTag==1027) {
            isLeader = 1;
        }
        else{
            isLeader = 0;
        }
        [activityDict setObject:@(isLeader) forKey:@"isLeader"];
        [activityDict setObject:@(self.companyId) forKey:@"createCompanyId"];
    ...
    if (self.redArray.count<=0) {
        //
        [designDict setObject:@"0" forKey:@"couponId"];
        [designDict setObject:@"0" forKey:@"giftCouponId"];
    }
    else if (self.redArray.count==1){
        ZCHCouponModel *model = self.redArray.firstObject;
        if ([model.type integerValue]==2) {
            [designDict setObject:model.couponId forKey:@"giftCouponId"];
            [designDict setObject:@"0" forKey:@"couponId"];
        }
        else{
            [designDict setObject:@"0" forKey:@"giftCouponId"];
            [designDict setObject:model.couponId forKey:@"couponId"];
        }
    }
    ...
    [dataDict setObject:@(self.companyId) forKey:@"companyId"];
        ...
        if (responseObj) {
            ...
            if (statusCode==1000) {
                if (self.isFistr) {
                    ...
                    for (UIViewController *vc in self.navigationController.childViewControllers) {
                        if ([vc isKindOfClass:[ShopDetailController class]]) {
                            ((ShopDetailController *)vc).model.noVipDesignId = (NSString *)responseObj[@"designsId"];
                            [self.navigationController popToViewController:vc animated:YES];
                        }
                    }
                }
                ...
                [[PublicTool defaultTool] publicToolsHUDStr:@"该公司保证金金额不足" controller:self sleep:1.5];
