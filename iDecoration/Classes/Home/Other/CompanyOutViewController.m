...
@interface CompanyOutViewController ()<UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate, CLLocationManagerDelegate, YellowPageCompanyTableViewCellDelegate, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
...
@property (strong, nonatomic) ZCHRightImageBtn *adderssbtn;
...
// 被分享的那个model
@property (nonatomic, strong) CompanyListModel *shareModel;
// 分享的公司ID
...
@property (nonatomic, assign) NSInteger pageNum;// 分页页码
@end

@implementation CompanyOutViewController
...
- (void)createUI {
    ...
    self.adderssbtn = [[ZCHRightImageBtn alloc] initWithFrame:CGRectMake(0, 0, 80, 60)];
    [self.adderssbtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.adderssbtn addTarget:self action:@selector(addressClick) forControlEvents:UIControlEventTouchUpInside];
    [self.adderssbtn setImage:[UIImage imageNamed:@"downArrow"] forState:UIControlStateNormal];
    if (IPhone4) {
        
        self.adderssbtn.myFont = [UIFont systemFontOfSize:12];
    } else if (IPhone5) {
        self.adderssbtn.myFont = [UIFont systemFontOfSize:14];
    } else {
        self.adderssbtn.myFont = [UIFont systemFontOfSize:14];
    }
    UIBarButtonItem *rightbarItem = [[UIBarButtonItem alloc] initWithCustomView:self.adderssbtn];
    self.navigationItem.rightBarButtonItem = rightbarItem;
    [self.adderssbtn setTitle:self.cityModel ? self.cityModel.name : @"北京" forState:UIControlStateNormal];
}

#pragma mark - 请求公司列表

- (void)requestCompanyListIn{
   ...
   //   NSString *api = [BASEURL stringByAppendingString:@"company/ getCompanyByType.do"];

    //0:距离,-1好评,-2信用,2浏览,1:案例,5:商品
    
    NSDictionary *paramDic = @{
                               @"longitude" : self.longitudety?:@(116.32),
                               @"latitude" : self.latitudety?:@(39.00),
                               @"serchContent" : self.searchTF.text,
                              
                               @"provinceId" : self.cityModel.pid?:@"0",
                               ...
                               @"merchantType" : @(self.type).stringValue ,
          ...
          //  [self setsystemlabarr:systemArray];
            ...
            if (responseObj[@"data"][@"commodityList"]) {
                ...
                if (self.pageNum == 1) {
                    ...
                    [self.adderssbtn setTitle:self.cityModel.name forState:UIControlStateNormal];
...
- (void)requestCompanyListInfo{
    ...
    NSDictionary *para = @{
                           @"longitude" : self.longitudety,
                           @"latitude" : self.latitudety,
...
- (void)addressClick {
    ...
    locationVC.refreshBlock = ^(NSDictionary *modelDic) {
        [weakSelf.adderssbtn setTitle:[modelDic objectForKey:@"name"]?:@
                            "" forState:UIControlStateNormal];
        weakSelf.pageNum = 1;
        [weakSelf requestCompanyListInfo];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HomeDefaultModel *model = self.dataArray[indexPath.row];
...
//    }
    ...
    if (isVIP) {
        ...
        company.companyName = model.companyName?:model.merchantName;
        company.companyID =model.companyId?:model.merchantId;
        //model.companyId?[NSNumber numberWithInteger:model.companyId].stringValue:[NSNumber numberWithInteger:model.merchantId].stringValue;
      //  [NSString stringWithFormat:@"%ld",(long)model.companyId?:(long)model.merchantId];
       
        company.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:company animated:YES];
    }else{
        VIPExperienceShowViewController *company = [VIPExperienceShowViewController new];
      //  controller.isEdit = false;
       
        
        company.isEdit=false;
        company.companyName = model.typeName?:model.merchantName;
        company.companyId = model.companyId?:model.merchantId;
...
#pragma mark -  YellowPageCompanyTableViewCellDelegate  点击三点事件
- (void)YellowPageCompanyTableViewCell:(YellowPageCompanyTableViewCell *)cell moreBtnClicked:(UIButton *)btn {
    ...
    self.shareModel = self.dataArray[shareIndexP.row];
    
    NSArray *shareTitleArray;
    if (self.shareModel.collectionId > 0) {
        shareTitleArray = [NSArray arrayWithObjects:@"取消收藏", @"投诉", @"分享", @"取消", nil];
    } else {
        shareTitleArray = [NSArray arrayWithObjects:@"收藏", @"投诉", @"分享", @"取消", nil];
    }
    ...
    if (self.shareModel.collectionId > 0) {
        ...
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"收藏" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            if (!isLogin) { // 未登录
                ...
                [self saveShopOrCompanyWithCompanyID:btn.tag];;
            }
        }];
        [alertC addAction:action];
    }
...
- (void)saveShopOrCompanyWithCompanyID:(NSInteger)companyId  {
    ...
    [NetManager afGetRequest:requestString parms:params finished:^(id responseObj) {
        ...
        if ([responseObj[@"code"] isEqualToString:@"1000"]) {
            self.shareModel.collectionId = [responseObj[@"collectionId"] integerValue];
...
- (void)unCollectionShopOrCompany {
    ...
    NSDictionary *paramDic = @{
                               @"collectionId" : @(self.shareModel.collectionId)
                               };
...
- (void)didClickShareContentBtn:(UIButton *)btn {
    NSString *shareTitle = self.shareModel.companyName;
//    NSString *descriptionStr = [NSString stringWithFormat:@"%@\n案例%@ 设计师%ld 工长%@ 监理%ld",self.shareModel.landline, self.shareModel.caseTotla, (long)self.shareModel.designerTotal, self.shareModel.foremanTotal, (long)self.shareModel.supervisorTotal];
    NSString *descriptionStr = self.shareModel.companyIntroduction;
...
//        UIImage *ima = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:self.shareModel.companyLogo]]];
//        UIImage *shareImage;
//        UIImageView *testImageView = [[UIImageView alloc] init];
//        [testImageView sd_setImageWithURL:[NSURL URLWithString:self.shareModel.companyLogo]];
//        if (!testImageView.image) {
...
//        } else {
//            shareImage = [UIImage imageWithData:[NSObject imageWithImage:ima scaledToSize:CGSizeMake(300, 300)]];
//        }
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        _tableView.tableFooterView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, BLEJWidth, 50)];
    }
    return _tableView;
}
