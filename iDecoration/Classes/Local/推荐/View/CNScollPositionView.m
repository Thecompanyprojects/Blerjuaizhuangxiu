...
-(void)setUp{
    self.titleViewWidth=kSCREEN_WIDTH/3;
...
-(void)layoutSubviews{
    ...
    for (int i = 0 ; i<self.titleContentView.subviews.count; i++) {
        ...
        button.frame = CGRectMake(i*self.titleViewWidth, 2, self.titleViewWidth, self.scrollView.bounds.size.height-2);
        if (button.selected) {
            if (i==0) {
...
-(void)drawRect:(CGRect)rect{
    ...
    CGContextSetStrokeColorWithColor(context, [UIColor hexStringToColor:@"FF7F00"].CGColor);
    if (_scroll) {
        if (self.scrollX>0) {//向左滑
            if (!self.moveLeft) {
                CGContextMoveToPoint(context, self.indexPoint.x-self.fixedWidth/2,0);
                CGContextAddLineToPoint(context, self.indexPoint.x+self.fixedWidth/2, 0);
                CGContextDrawPath(context, kCGPathStroke);
                return;
            }
            CGContextMoveToPoint(context, self.indexPoint.x+self.fixedWidth/2,0);
            CGContextAddLineToPoint(context, self.indexPoint.x-self.scrollX, 0);
        }
        else{//向右滑
            if (!self.moveRight) {
                CGContextMoveToPoint(context, self.indexPoint.x-self.fixedWidth/2,0);
        ...
        CGContextMoveToPoint(context, self.indexPoint.x-self.fixedWidth,0);
        CGContextAddLineToPoint(context, self.indexPoint.x+self.fixedWidth, 0);
    }
    CGContextDrawPath(context, kCGPathStroke);
}
