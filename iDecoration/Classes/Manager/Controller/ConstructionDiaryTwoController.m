//
//  ConstructionDiaryTwoController.m
//  iDecoration
//
...
@interface ConstructionDiaryTwoController ()<UITableViewDelegate,UITableViewDataSource,DiaryTitleHeaderViewDelegate,SignContractTableViewCellDelegate,UIScrollViewDelegate,UITextViewDelegate,CaseMaterialCellDelegate,SDCycleScrollViewDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UIAlertViewDelegate,SiteInfoTableViewCellDelegate>
{
    ...
    NSInteger deleteCaseIndex;//删除哪一个本案主材
...
@implementation ConstructionDiaryTwoController
...
- (void)viewDidLoad {
    ...
    //添加本案主材成功
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reciveNotificationGetDiaryData:) name:@"AddCaseMSucess" object:nil];
    
    //修改施工日志的工地信息的通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reciveNotificationGetDiaryData:) name:@"modifyContrctInfo" object:nil];
    
    //编辑施工日志的节点通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshShowCell:) name:@"editContrutionNode" object:nil];
    
    //添加施工日志节点通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshShowCell:) name:@"addContructionNode" object:nil];
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(refreshShowCell:)name:@"UITextFieldTextDidChangeNotification"
...
- (void)addTwoDimensionCodeView {
    ...
    self.TwoDimensionCodeView.areaLabel.hidden = true;
...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    __weak ConstructionDiaryTwoController *weakSelf = self;
    ...
    if (section >=2) {
        ...
        headerView.tapBlock = ^(){
            ...
            BOOL isLogin = [[PublicTool defaultTool]publicToolsJudgeIsLogined];
            if ((self.siteModel.ccComplete==0||self.siteModel.ccComplete==1)&&(isExit)&&(isLogin)){
                ...
                //当前节点是交工展示  --先判断有没有数据 有：无操作  无：判断上面的所有节点是否添加过（添加过：添加交工展示 没添加：提示添加上面的节点）
                else if(selectTag==2011){
                    if (node.journalList.count<=0) {
                        BOOL allIsAdd = NO;
                        NSMutableArray *temNodeArray = [NSMutableArray array];
                        [temNodeArray addObjectsFromArray:self.nodeArr];
                        [temNodeArray removeObjectsInRange:NSMakeRange(9, 2)];
                        for (NodeModel *tempNode in temNodeArray) {
                            if (tempNode.journalList.count<=0) {
                                allIsAdd = NO;
                                break;
                            }
                            else{
                        ...
                        if (!allIsAdd) {
                            //没添完：提示添加上面的节点
                            [[PublicTool defaultTool] publicToolsHUDStr:@"无法操作此节点，您还有未完成的节点" controller:self sleep:1.5];
                        }
                        else{
                            //添加完：添加交工展示
                            selectNodeIsHave = NO;
                    ...
                    if (selectTag==2001) {
                        ...
                        NodeModel *firstModel = self.nodeArr.firstObject;
                        if (firstModel.journalList.count<=0) {
                            [[PublicTool defaultTool] publicToolsHUDStr:@"签订合同未填写" controller:self sleep:1.5];
                        }
                        else{
                            if (node.journalList.count<=0) {
                                selectNodeIsHave = NO;
        ...
        if ((self.siteModel.ccComplete==0||self.siteModel.ccComplete==1)&&(isExit)&&(isLogin)) {
            ...
            if ([model.crNodeNumber integerValue]==2010) {
                ...
                //判断当前节点是否有数据 - 无：不显示我来添加  有：再判断自己是否添加过 添加过：不显示我来添加  没添加过：根据xsFlag判断自己是否有权限
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        SiteInfoTableViewCell *cell = [SiteInfoTableViewCell cellWithTableView:tableView];
        ...
        bool isLogin = [[PublicTool defaultTool]publicToolsJudgeIsLogined];
        //经理，总经理，创建人，可以编辑工地，并且未交工，并且登录
        if ((self.siteModel.ccHouseholderId == user.agencyId||isZManage||isManage||implement)&&(self.siteModel.ccComplete==0||self.siteModel.ccComplete==1)&&isLogin) {
            cell.modifyBtn.hidden = NO;
        }
        else{
            cell.modifyBtn.hidden = YES;
        }
        siteInfoCellH = cell.cellH;
...
//            monitoringVC *vc = [monitoringVC new];
//            [self.navigationController pushViewController:vc animated:YES];
...
#pragma mark 本案主材
        ...
        BOOL isLogin = [[PublicTool defaultTool]publicToolsJudgeIsLogined];
        if ((self.siteModel.ccComplete==2||self.siteModel.ccComplete==3)||(!isLogin)||(!isExit)){
        ...
        if (arr.count) {
            ...
            BOOL isLogin = [[PublicTool defaultTool]publicToolsJudgeIsLogined];
            [cell configData:dict indexpath:indexPath isComplete:self.siteModel.ccComplete isLogin:isLogin isExit:isExit type:1 nodeName:@""];
            //                cellH = cell.cellH;
            NSString *indexSectionRow = [NSString stringWithFormat:@"%ld%ld",(long)indexPath.section,(long)indexPath.row];
            
            //                self.cellHDict = cell.cellHDict;
...
-(void)addPointWith:(NSInteger)tag{
    ...
    if (selectTag==2010) {
        ...
        signVC.type = [NSString stringWithFormat:@"%ld",selectTag];
        signVC.isAdd = n;
        signVC.index = 1;
        signVC.nodeId = model.crNodeNumber;
        signVC.companyLogo = self.siteModel.companyLogo;
        signVC.companyName = self.siteModel.ccBuilder;
...
#pragma mark - SiteInfoTableViewCellDelegate
...
-(void)deleteShopWith:(NSIndexPath *)path{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"是否确认删除该主材日志？"
                                                        message:@""
                                                       delegate:self
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (alertView.tag==100) {
        if (buttonIndex==1) {
            CaseMaterialTwoModel *model = self.caseDataArray[deleteCaseIndex];
    ...
    if (alertView.tag==200) {
        if (buttonIndex==1) {
            ...
            NSString *journalId = [[journalList[_deletePath.row] objectForKey:@"cblejJournalModel"] objectForKey:@"journalId"];
        ...
        if (responseObj && (NSInteger)responseObj[@"code"] != 10004) {
            ...
            switch (statusCode) {
                    ...
                    
                {
                    ...
                    BOOL isHavePower = NO;
                    if (isDesiger||isZManage||isManage||implement) {
                        isHavePower = YES;
                    }
                    else{
                        isHavePower = NO;
                            ...
                            if (statusCode==1000) {
                                ...
                                if (arr.count<=0) {
                                    ...
                                    if (isHavePower) {
                                        ...
                                        DesignCaseListController *designVC = [DesignCaseListController new];
                                        designVC.consID = self.consID;
                                        designVC.isPower = isHavePower;
                                        ...
                                        designVC.companyName = self.siteModel.ccBuilder;
                                        ...
                                        [[PublicTool defaultTool] publicToolsHUDStr:@"本案设计还未上传，请耐心等待" controller:self sleep:1.5];
                                    }
                                }
                                else{
                               ...
                               [[PublicTool defaultTool] publicToolsHUDStr:@"参数错误" controller:self sleep:1.5];
                                ...
                                [[PublicTool defaultTool] publicToolsHUDStr:@"查询失败" controller:self sleep:1.5];
                            }
...
//                                [[PublicTool defaultTool] publicToolsHUDStr:@"业主还未上传数据，请耐心等待" controller:self sleep:1.5];
        ...
        if (responseObj && (NSInteger)responseObj[@"code"] != 10004) {
            ...
            switch (statusCode) {
                    ...
                    
                {
                    ...
                    implement = [[[responseObj objectForKey:@"data"]objectForKey:@"implement"] boolValue];
            ...
            switch (statusCode) {
                    ...
                    
                {
                    ...
                    [UIView animateWithDuration:0 animations:^{
                        ...
                        [self.diaryTableView reloadSections:indexSet2 withRowAnimation:UITableViewRowAnimationNone];
                    }];
        ...
        if (responseObj && (NSInteger)responseObj[@"code"] != 10004) {
            ...
            switch (statusCode) {
                    ...
                    
                {
                    ...
                    [self.caseDataArray removeObjectAtIndex:deleteCaseIndex];
                    
                    //一个section刷新
                    NSIndexSet *indexSet=[[NSIndexSet alloc]initWithIndex:11];
                    [UIView animateWithDuration:0 animations:^{
                        [self.diaryTableView reloadSections:indexSet withRowAnimation:UITableViewRowAnimationFade];
                    }];
...
-(void)rewardAction:(NSIndexPath *)path{
    ...
    [[PublicTool defaultTool] publicToolsHUDStr:@"此功能暂未开放" controller:self sleep:1.0];
}
