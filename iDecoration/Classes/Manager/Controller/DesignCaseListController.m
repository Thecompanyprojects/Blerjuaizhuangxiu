...
@interface DesignCaseListController ()<UITableViewDelegate,UITableViewDataSource,DesignCaseListMidCellDelegate,DesignCaseListHeadCellDelegate,UIGestureRecognizerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,SSPopupDelegate,ASProgressPopUpViewDataSource,UIActionSheetDelegate,VrCommenCellDelegate,VoteCommenCellDelegate,RedCommenCellDelegate>{
...
@implementation DesignCaseListController
...
//    self.title = @"本案设计";
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(receiveAddTextData:) name:@"addTextDesign" object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(receiveEditTextData:) name:@"editTextDesign" object:nil];
    
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(receiveAddTextToBottomData:) name:@"addTextToBottomDesign" object:nil];
...
//        [self.hiddenV removeAllSubViews];
...
-(void)successBtnClick:(UIButton *)btn{
    if (self.coverTitle.length<=0) {
        [[PublicTool defaultTool] publicToolsHUDStr:@"标题不能为空" controller:self sleep:1.5];
        return;
    }
    [self pushData];
}
...
//        DesignCaseListModel *firstModel = self.dataArray.firstObject;
    ...
    if (self.isFistr) {
        ...
        [designDict setObject:@(self.consID) forKey:@"constructionId"];
        ...
        if (!self.voteDescribe||self.voteDescribe.length<=0) {
            ...
            [voteDict setObject:@"" forKey:@"voteDescribe"];
            [voteDict setObject:@"23232" forKey:@"voteEndTime"];
            [voteDict setObject:@(1) forKey:@"voteType"];
    ...
    NSString *defaultApi = [BASEURL stringByAppendingString:@"designs/saveDesign.do"];
    ...
    [NetManager afPostRequest:defaultApi parms:paramDic finished:^(id responseObj) {
        ...
        if (responseObj) {
            ...
            if (statusCode==1000) {
                self.isFistr = NO;
                BOOL isKindWebClass = NO;//是否是从展示页面进入的
                for (UIViewController *vc in self.navigationController.viewControllers) {
                    if ([vc isKindOfClass:[NewDesignImageWebController class]]) {
                        isKindWebClass = YES;
                        break;
                    }
                    else{
                ...
                if (isKindWebClass) {
                    //从展示页面进入的
                    [self requestNewCaseDesignWith:isKindWebClass];
                }
                else{
                    //不是从展示页面进入的
                    [self requestNewCaseDesignWith:isKindWebClass];
...
-(void)requestNewCaseDesignWith:(BOOL)isKindWebClass{
        ...
        if (responseObj && (NSInteger)responseObj[@"code"] != 10004) {
            ...
            if (statusCode==1000) {
                ...
                if (arr.count<=0) {
                    ...
                    if (isKindWebClass) {
                        [[NSNotificationCenter defaultCenter]postNotificationName:@"refreshNewDesignWebVc" object:nil];
...
//                        [self.navigationController.view.layer addAnimation:transition forKey:nil];
                        ...
                        vc.fromIndex = self.fromIndex;
                        vc.isPower = self.isPower;
                        vc.isComplate = self.isComplate;
                        vc.isFistr = self.isFistr;
                        vc.musicStyle = self.musicStyle;
                        
                        vc.coverTitle = self.coverTitle;
                        vc.coverTitleTwo = self.coverTitleTwo;
                        vc.coverImgUrl = self.coverImgUrl;
