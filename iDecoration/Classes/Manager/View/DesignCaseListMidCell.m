...
@implementation DesignCaseListMidCell
...
+(instancetype)cellWithTableView:(UITableView *)tableView path:(NSIndexPath *)path
{
    ...
    DesignCaseListMidCell *cell = [tableView dequeueReusableCellWithIdentifier:TextFieldCellID];
    if (cell == nil) {
        cell =[[DesignCaseListMidCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TextFieldCellID];
    }
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.backView addSubview:self.plachoderVideoImgV];
...
-(void)configWith:(BOOL)isHidden data:(id)data isHaveDefaultLogo:(BOOL)isHaveDefaultLogo{
    if ([data isKindOfClass:[DesignCaseListModel class]]) {
        DesignCaseListModel *model = data;
        
//        if(model.videoUrl&&model.videoUrl.length>0){
//            self.photoImgV.image = [UIImage imageNamed:@"edit_photo"];
//        }
//        else{
//            if (isHaveDefaultLogo) {
//                [self.photoImgV sd_setImageWithURL:[NSURL URLWithString:model.imgUrl] placeholderImage:[UIImage imageNamed:@"edit_text_icon_normal"]];
//            }
//
//
//            else{
//                [self.photoImgV sd_setImageWithURL:[NSURL URLWithString:model.imgUrl] placeholderImage:[UIImage imageNamed:DefaultIcon]];
//            }
//        }
        ...
        CGSize textSize = [model.content boundingRectWithSize:CGSizeMake(self.backView.width-self.photoImgV.right-10-30, self.photoImgV.height)
        ...
        BLEJPackageArticleDesignModel *model = data;
...
-(UIButton *)deleteBtn{
    if (!_deleteBtn) {
        ...
        _deleteBtn.frame = CGRectMake(5, 5, 25, 25);
...
-(UIImageView *)plachoderVideoImgV{
    if (!_plachoderVideoImgV) {
        CGFloat width = 80;
        _plachoderVideoImgV = [[UIImageView alloc]initWithFrame:CGRectMake(self.photoImgV.left+(self.photoImgV.width/2-width/2), self.photoImgV.top+(self.photoImgV.width/2-width/2), width, width)];
        _plachoderVideoImgV.image = [UIImage imageNamed:@"edit_photo"];
        _plachoderVideoImgV.userInteractionEnabled = YES;
        
        
        UITapGestureRecognizer *ges = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(photoImgVClick:)];
        [_plachoderVideoImgV addGestureRecognizer:ges];
