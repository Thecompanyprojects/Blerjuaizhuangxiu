...
-(NSMutableArray *)GetVedioAssets:(PHFetchResult *)fetchResult
{
    NSMutableArray *dataArray = [NSMutableArray array];
    for (PHAsset *asset in fetchResult) {
        //只添加图片类型资源，去除视频类型资源
...
-(PHFetchResult *)GetCameraRollFetchResul
{
    ...
    PHFetchResult *fetch = [[PHFetchResult alloc]init];
    
    if (smartAlbumsFetchResult.count != 0) {
        
        fetch = [PHAsset fetchAssetsInAssetCollection:[smartAlbumsFetchResult objectAtIndex:0] options:nil];
    }
