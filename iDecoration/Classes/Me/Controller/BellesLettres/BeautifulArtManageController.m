//
//  BeautifulArtManageController.m
//  iDecoration
//
...
@interface BeautifulArtManageController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate,SSPopupDelegate>
{
...
@implementation BeautifulArtManageController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadList) name:@"refreshBeatAteList" object:nil];
}
...
//            vc.companyLogo = self.companLogo;
            vc.activityTime = self.activityAddress;
...
-(void)refreshPreviousVc{
    if (self.BeautifulArtManageBlock) {
        self.BeautifulArtManageBlock();
    }
}
...
//    [self.TwoDimensionCodeView.companyIcon sd_setImageWithURL:[NSURL URLWithString:@""] placeholderImage:nil options:SDWebImageRetryFailed];
    self.TwoDimensionCodeView.companyIcon.hidden = YES;
    self.TwoDimensionCodeView.activityTime = self.activityTime;
    if (self.activityAddress.length<=0) {
        self.activityAddress = @"线上活动";
    }
