...
@interface AddAllSubPromotionController ()<UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate>
...
@implementation AddAllSubPromotionController
...
//    AddPromotionCell *cell = [self.tableView cellForRowAtIndexPath:indexPath];
    ...
    if ([totalMoney isEqualToString:@""]) {
        [[UIApplication sharedApplication].keyWindow hudShowWithText:@"请输入消费金额"];
        return;
    }
    
    if ([subMoney isEqualToString:@""]) {
        [[UIApplication sharedApplication].keyWindow hudShowWithText:@"请输入满减金额"];
        return;
    }
    
    if (subMoney.doubleValue >= totalMoney.doubleValue) {
        [[UIApplication sharedApplication].keyWindow hudShowWithText:@"要减的价格要大于消费的价格"];
        return;
    }
    ...
    if (self.isSectionGoods) {
        [paramDic setObject:@(4) forKey:@"activityType"];
    } else {
        [paramDic setObject:@(3) forKey:@"activityType"];
    }
...
//    [paramDic setObject:@(totalMoney.doubleValue) forKey:@"consumeMoney"];
//    [paramDic setObject:@(subMoney.doubleValue) forKey:@"discountMoney"];
    
    [paramDic setObject:totalMoney forKey:@"consumeMoney"];
    [paramDic setObject:subMoney forKey:@"discountMoney"];
...
//            cell.scrollView.contentSize = cell.scrollView.frame.size;
            ...
            cell.numLabel.text = [NSString stringWithFormat:@"共%ld件", self.selectGoodsImageArray.count];
            return cell;
        } else {
        ...
        if (indexPath.section == 1) {
            ...
            cell.textField.delegate = self;
            if (indexPath.row == 2) {
                cell.textField.text = _isSectionGoods ? @"单品满减" : @"全店满减";
