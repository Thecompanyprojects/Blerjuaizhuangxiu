//
//  NewEditGoodsParameterViewController.m
//  iDecoration
//
...
@interface NewEditGoodsParameterViewController ()<UITableViewDelegate, UITableViewDataSource, UITextViewDelegate, UITextFieldDelegate, NewCustomParameterItemViewDelegate, EditGoodsParameterCellDelegate>

@property (nonatomic, strong) UITableView *tableView;

@property (nonatomic, strong)  NewCustomParameterItemView *customerItemView;

@end

@implementation NewEditGoodsParameterViewController
...
- (void)addMoreAction {
    [self.view endEditing:YES];
    if (self.regularArray.count >0) {
        self.customerItemView.regularArray = [self.regularArray mutableCopy];
    } else {
        self.customerItemView.regularArray = [@[] mutableCopy];
    }
    self.customerItemView.isImplementOrGeneralManager = self.isImplementOrGeneralManager;
...
#pragma mark - NewCustomParameterItemViewDelegate
...
- (void)newParameterItemViewAddItemAction:(NewCustomParameterItemView *)parameterItemView {
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        parameterItemView.listArray = [self.listArray mutableCopy];
...
- (void)newParameterItemViewDeleteItemAction:(NewCustomParameterItemView *)parameterItemView withTitleName:(NSString *)title {
    [self.listArray enumerateObjectsUsingBlock:^(GoodsParamterModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj.name isEqualToString:title]) {
            [self.listArray removeObject:obj];
            *stop = YES;
        }
    }];
    if (self.isImplementOrGeneralManager && [self.regularArray containsObject:title]) {
        [self.regularArray removeObject:title];
    }
    parameterItemView.regularArray = self.regularArray;
    parameterItemView.listArray = [self.listArray mutableCopy];
    [parameterItemView layoutSubviews];
    [self.tableView reloadData];
}
...
- (void)deleteActionWithModel:(GoodsParamterModel *)model atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isImplementOrGeneralManager && [self.regularArray containsObject:model.name]) {
        [self.regularArray removeObject:model.name];
    }
...
- (NewCustomParameterItemView *)customerItemView {
    if (_customerItemView== nil) {
        _customerItemView= [[NewCustomParameterItemView alloc] initWithFrame:CGRectMake(0, kSCREEN_HEIGHT, kSCREEN_WIDTH, kSCREEN_HEIGHT - 64)];
        _customerItemView.delegate = self;
        [self.view addSubview:_customerItemView];
        [self.view sendSubviewToBack:self.tableView];
        
        MJWeakSelf;
        _customerItemView.finishBlock = ^(NewCustomParameterItemView *promptView) {
