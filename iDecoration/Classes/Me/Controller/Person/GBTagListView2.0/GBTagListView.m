...
-(void)setTagWithTagArray:(NSArray*)arr{
    ...
    [arr enumerateObjectsUsingBlock:^(NSString*str, NSUInteger idx, BOOL *stop) {
        ...
        if(_canTouch){
            ...
            if (idx==arr.count-1) {
                [tagBtn addTarget:self action:@selector(addbtnclick) forControlEvents:UIControlEventTouchUpInside];
            }
            else
            {
...
//        tagBtn.clipsToBounds=YES;
        ...
        [tagBtn setTitleColor:kRandomColor forState:(UIControlStateNormal)];
        [tagBtn setBackgroundColor:[UIColor clearColor] forState:(UIControlStateNormal)];
        CAShapeLayer *layer   = [[CAShapeLayer alloc] init];
        layer.frame            = CGRectMake(0, 0 , tagBtn.frame.size.width, tagBtn.frame.size.height);
        ...
        [tagBtn.layer addSublayer:layer];
    }];
...
-(void)tagBtnClick:(UIButton*)button{
    ...
    if(button.selected==YES){
        button.backgroundColor=[UIColor hexStringToColor:@"BBBDD1"];
