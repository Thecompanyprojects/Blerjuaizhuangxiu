...
@interface ActivityManageController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate,SSPopupDelegate>
{
...
@implementation ActivityManageController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadList) name:@"refreshActivityList" object:nil];
}
...
//        cell.collectL.text = self.collectionNum;
    ...
    if (indexPath.section==1) {
        ...
        cell.manageActBlock = ^{
            ActivitySignUpManageController *vc = [[ActivitySignUpManageController alloc]init];
...
//                        [self.navigationController pushViewController:payVC animated:YES];
        ...
        cell.lookActBlock = ^{
            ...
            vc.companyLogo = self.companLogo;
            vc.activityAddress = self.activityAddress;
            vc.activityTime = self.startTime;
...
#pragma mark - 开通会员之后刷新上一个页面的数据

-(void)refreshPreviousVc:(NSInteger)n{
    if (self.manageBlock) {
        self.manageBlock(n);
    }
}
    ...
    [NetManager afPostRequest:defaultApi parms:paramDic finished:^(id responseObj) {
        ...
        if (responseObj && (NSInteger)responseObj[@"code"] != 10004) {
            ...
            if (statusCode==1000) {
                ...
                NSString *startStr = [NSString stringWithFormat:@"%@",[activityDict objectForKey:@"startTime"]];
                self.startTime = [self timeWithTimeIntervalString:startStr];
...
-(void)requestColseSignUp{
    UserInfoModel *user = [[PublicTool defaultTool] publicToolsGetUserInfoModelFromDict];
    NSString *defaultApi = [BASEURL stringByAppendingString:@"cblejActivity/stopSignUp.do"];
    ...
    [NetManager afPostRequest:defaultApi parms:paramDic finished:^(id responseObj) {
        ...
        if (responseObj && (NSInteger)responseObj[@"code"] != 10004) {
            ...
            if (statusCode==1000) {
                //                NSArray *arr = [NSArray yy_modelArrayWithClass:[CaseMaterialTwoModel class] json:caseArr];
                [[PublicTool defaultTool] publicToolsHUDStr:@"关闭成功" controller:self sleep:1.5];
...
- (void)setQRShareOfPerson{
    ...
    [self.TwoDimensionCodeView.companyIcon sd_setImageWithURL:[NSURL URLWithString:self.companLogo] placeholderImage:nil options:SDWebImageRetryFailed];
    self.TwoDimensionCodeView.activityTime = self.startTime;
...
- (void)didClickShareContentBtn:(UIButton *)btn {
    ...
    NSString *temStr;
    if (!self.designSubTitle||self.designSubTitle.length<=0) {
        temStr = @" ";
    }
    else{
        temStr = self.designSubTitle;
    }
        NSString *shareDescription = temStr;
        NSURL *shareImageUrl;
        UserInfoModel *user = [[PublicTool defaultTool] publicToolsGetUserInfoModelFromDict];
        if (self.coverMap) {
        ...
        switch (btn.tag) {
            ...
            {// 微信好友
                ...
                if (!_tempAgenceId) {
                    shareURL = [BASEHTML stringByAppendingString:[NSString stringWithFormat:@"api/cblejActivity/%@/%@/%ld.htm",self.activityId,companyStr, (long)0]];
                }
                else{
                    shareURL = [BASEHTML stringByAppendingString:[NSString stringWithFormat:@"api/cblejActivity/%@/%@/%ld.htm",self.activityId,companyStr, (long)user.agencyId]];
                }
...
- (void)shareNumberCount {
    ...
    [param setObject:agencyID forKey:@"agencyId"];
