//
//  CreatActivityController.m
//  iDecoration
//
//  Created by sty on 2017/10/20.
...
#import "ActivityChangeAssistantTitleController.h"
...
@interface CreatActivityController ()<UITableViewDelegate,UITableViewDataSource,DesignCaseListMidCellDelegate,DesignCaseListHeadCellDelegate,UIGestureRecognizerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,SSPopupDelegate,ASProgressPopUpViewDataSource,UIActionSheetDelegate,VrCommenCellDelegate,VoteCommenCellDelegate>{
...
@implementation CreatActivityController
...
-(void)changeCoverTitleTwo{
    ActivityChangeAssistantTitleController *vc = [[ActivityChangeAssistantTitleController alloc]init];
    __weak typeof (self) wself = self;
    vc.strBlock = ^(NSString *str) {
...
-(void)addToActivity{
    ...
    vc.activityType = ActivityTypeUnion;
...
//        }];
    ...
    //        [UIView animateWithDuration:0 animations:^{
    //
    ////        [self.tableView reloadTableViewWithRow:path.row andSection:1];
    ////        [self.tableView reloadTableViewWithRow:path.row - 1 andSection:1];
    //
    //            [self.tableView reloadSections:indexSet withRowAnimation:UITableViewRowAnimationNone];
    //        }];
    //    }];
    //    NSIndexPath *pathTwo = [NSIndexPath indexPathForRow:path.row-1 inSection:path.section];
    //    [self.tableView moveRowAtIndexPath:path toIndexPath:pathTwo];
    //
    //
...
//        }];
    ...
    //            [self.tableView reloadSections:indexSet withRowAnimation:UITableViewRowAnimationNone];
    //        }];
    ////        id data = self.dataArray[path.row];
    ////        [self.dataArray removeObjectAtIndex:path.row];
    ////        [self.dataArray insertObject:data atIndex:path.row + 1];
    ////        [self.tableView reloadData];
    ////        [self.tableView reloadData];
    //    }];
    //    NSIndexPath *pathTwo = [NSIndexPath indexPathForRow:path.row+1 inSection:path.section];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (alertView.tag==200) {
        if (buttonIndex==1) {
            ...
            if (isFromSetWater){
                for (UIViewController *vc in self.navigationController.childViewControllers) {
                    if ([vc isKindOfClass:[UnionActivityListController class]]) {
...
-(void)pushData{
    ...
    if (self.isFistr) {
        ...
        [activityDict setObject:@(0) forKey:@"isLeader"];
    ...
    NSString *defaultApi = [BASEURL stringByAppendingString:@"cblejActivity/save.do"];
