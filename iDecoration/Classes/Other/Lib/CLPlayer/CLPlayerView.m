...
- (NSString *)playerCurrentTime {
    //当前时长
    NSInteger proMin                    = (NSInteger)CMTimeGetSeconds([_player currentTime]) / 60;//当前秒
    NSInteger proSec                    = (NSInteger)CMTimeGetSeconds([_player currentTime]) % 60;//当前分钟
    return [NSString stringWithFormat:@"%02ld:%02ld", (long)proMin, (long)proSec];
}

- (void)timeStackTime:(void (^)(NSString *))timeBlock {
    if (_playerItem.duration.timescale != 0){
        //设置进度条
        self.maskView.slider.maximumValue   = 1;
        self.maskView.slider.value          = CMTimeGetSeconds([_playerItem currentTime]) / (_playerItem.duration.value / _playerItem.duration.timescale);
        ...
        NSInteger durMin                    = (NSInteger)_playerItem.duration.value / _playerItem.duration.timescale / 60;//总分钟
        NSInteger durSec                    = (NSInteger)_playerItem.duration.value / _playerItem.duration.timescale % 60;//总秒
        NSString *timeStr = [NSString stringWithFormat:@"%02ld:%02ld", (long)durMin, (long)durSec];
        self.maskView.totalTimeLabel.text   = timeStr;
...
-(void)cl_playButtonAction:(UIButton *)button{
    if (button.selected == NO){
        ...
        if ([self.delegate respondsToSelector:@selector(playerViewPauseAction)]) {
            [self.delegate playerViewPauseAction];
        }
    }else{
        [self playVideo];
        
        if ([self.delegate respondsToSelector:@selector(playerViewPlayAction)]) {
            [self.delegate playerViewPlayAction];
        }
    }
    //重新添加工具条定时消失定时器
    self.toolBarDisappearTime = _toolBarDisappearTime;
