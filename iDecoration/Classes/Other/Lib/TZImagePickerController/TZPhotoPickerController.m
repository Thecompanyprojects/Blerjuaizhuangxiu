...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    //xulin 2016-08-02 此处有待测试，
//    NSString *type = [info objectForKey:UIImagePickerControllerOriginalImage];
//    if ([type isEqualToString:@"public.image"]) {
        TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
        [imagePickerVc showProgressHUD];
        UIImage *image = [info objectForKey:UIImagePickerControllerOriginalImage];
        CGSize scaleSize = CGSizeMake(image.size.width/2.5, image.size.height/2.5);
        UIImage *cmpImg = [self scaleImage:image toSize:scaleSize];
        [[TZImageManager manager] savePhotoWithImage:cmpImg completion:^{
