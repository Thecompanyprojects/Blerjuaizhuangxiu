...
- (instancetype)initWithFrame:(CGRect)frame withTitle:(NSString *)title
{
    ...
    if (self) {
        ...
        myLable.text = title;
        myLable.font = [UIFont systemFontOfSize:15.0f];
        myLable.backgroundColor = [UIColor clearColor];
        
        //计算文本的宽度
        CGFloat calcuWidth = [self widthForTextString:title height:labelHeight fontSize:15.0f];
        ...
        if (calcuWidth>frame.size.width) {
            ...
            behindLabel.backgroundColor = [UIColor clearColor];
            [labelArray addObject:behindLabel];
            [self addSubview:behindLabel];
            

            [self doCustomAnimation];
            [[NSNotificationCenter defaultCenter] removeObserver:self name:@"kPaoMaContinueAnimation" object:nil];
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(doCustomAnimation) name:@"kPaoMaContinueAnimation" object:nil];
...
- (instancetype)initWithFrame:(CGRect)frame withAttributeTitle:(NSAttributedString *)attrTitle
{
    self = [super initWithFrame:frame];
    ...
    if (self) {
        ...
        UILabel *myLable = [[UILabel alloc]init];
        myLable.attributedText = attrTitle;
        myLable.font = [UIFont systemFontOfSize:15.0f];
        myLable.backgroundColor = [UIColor clearColor];
        
        //计算文本的宽度
        CGFloat calcuWidth = [self widthForTextString:[attrTitle string] height:labelHeight fontSize:15.0f];
        ...
        if (calcuWidth>frame.size.width) {
            UILabel *behindLabel = [[UILabel alloc]init];
            behindLabel.frame = behindFrame;
            behindLabel.attributedText = attrTitle;
            behindLabel.font = [UIFont systemFontOfSize:15.0f];
            behindLabel.backgroundColor = [UIColor clearColor];
            [labelArray addObject:behindLabel];
            [self addSubview:behindLabel];
...
- (void)doCustomAnimation
{
    ...
    YSNLog(@"labelOne %ld", lableOne.text.length)
//    animation.duration = animationDuration;
    animation.duration = lableOne.text.length / 5.0;
    ...
    YSNLog(@"labelTwo %ld", lableTwo.text.length)
//    animationTwo.duration = animationDuration;
    animationTwo.duration = lableTwo.text.length / 5.0;
...
//- (CGFloat) widthForAttrString:(NSAttributedString *)tStr height:(CGFloat)tHeight fontSize:(CGFloat)tSize{
//
//    NSDictionary *dict = @{NSFontAttributeName:[UIFont systemFontOfSize:tSize]};
//    CGRect rect = [tStr boundingRectWithSize:CGSizeMake(MAXFLOAT, tHeight) options:NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin attributes:dict context:nil];
//    return rect.size.width+5;
