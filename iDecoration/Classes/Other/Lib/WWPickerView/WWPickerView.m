...
#define  WW_FONTSEIZ     [UIFont systemFontOfSize:16]
...
- (void)setDateViewWithTitle:(NSString *)title withMode:(UIDatePickerMode)mode;
{
    ...
    UILabel *titleLbl = [[UILabel alloc] initWithFrame:CGRectMake(40, 0, SCREENSIZE.width - 80, header.height)];
    titleLbl.text = title;
    titleLbl.textAlignment = NSTextAlignmentCenter;
    titleLbl.textColor = [UIColor blackColor];//[self getColor:@"FF8000"];
//    titleLbl.font = [UIFont fontWithName:@"Helvetica-Bold" size:17.0];
    titleLbl.font = WW_FONTSEIZ;
    [header addSubview:titleLbl];
    
    UIButton *submit = [[UIButton alloc] initWithFrame:CGRectMake(SCREENSIZE.width - 50, 0, 50 ,header.height)];
...
//    submit.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:15.0];
    submit.titleLabel.font = WW_FONTSEIZ;
    [submit addTarget:self action:@selector(submit:) forControlEvents:UIControlEventTouchUpInside];
    [header addSubview:submit];
    
    UIButton *cancel = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 50 ,header.height)];
    [cancel setTitle:@"取消" forState:UIControlStateNormal];
    [cancel setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
//    cancel.backgroundColor = [UIColor whiteColor];
//    cancel.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:15.0];
    cancel.titleLabel.font = WW_FONTSEIZ;
...
//            [formatter setDateFormat:@"yyyy-MM-dd HH:mm"];
    ...
    if (self.isNeedRow == YES) {
        blockTwo(selectedStr,selectRow);
    }
    else{
        block(selectedStr);
    }
