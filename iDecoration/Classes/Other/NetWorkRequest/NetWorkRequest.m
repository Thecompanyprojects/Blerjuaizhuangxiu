...
//    }
//    if (string.length > 0) {
//        if (![urlStr isEqualToString:@"user/log-off"]) {
...
//    }
    ...
    [manager POST:url parameters:dic constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        for (int i = 0; i < imageArray.count; i++) {
            ...
            [formData appendPartWithFileData:data name:name fileName:[NSString stringWithFormat:@"image%d.jpg", i] mimeType:@"image/jpeg"];
        }
...
+(void)upImaWithUrl:(NSString *)urlStr andPost:(NSDictionary *)dic pngArray:(NSArray *)pngArray jpgArray:(NSArray *)jpgArray success:(void (^)(id result))success fail:(void (^)(id error))fail
{
    ...
    [[AFHTTPSessionManager manager] POST:urlStr parameters:dic constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
