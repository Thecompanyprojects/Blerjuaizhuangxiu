...
- (CGFloat)setupLineAndVerticalLabel {
    ...
    float heightGap = (self.frame.size.height - maxHeight - VerticalLineGap) / 6.0;

    float highestValueEColumnChart = 500 * 1.1;
    float valueGap = highestValueEColumnChart / 5.0;
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (i == 0) {
            ...
            text = [NSString stringWithFormat:@"%d",(6 - i) * 100];
        }
...
- (void)logic {
    ...
    float heightGap = bottomLine.frame.origin.y / 6.0;
    for (int i = 0; i < 7; i++) {
    ...
    for (int i = 0; i < totalCount; i++) {
        ...
        //bar高 157
        CGFloat labelY = 0;
        CGFloat grade = ((1 - dataModel.value / maxValue) * self.frame.size.height);
        CGFloat height = self.frame.size.height - 3 - maxHeight - VerticalLineGap;
        UILabel *label = [UILabel new];
        label.text = [NSString stringWithFormat:@"%0.0f",dataModel.value];
        if (dataModel.value == 614) {
            NSLog(@"%f",grade);
        }
        if (dataModel.value > 500) {
            labelY = -20;
        }else
            labelY = grade - 40;
        label.frame = CGRectMake(barPosX, labelY, 40, 30);
        label.centerX =  bar.centerX;
        label.textAlignment = NSTextAlignmentCenter;
        NSLog(@"%f == value%0.0f",barPosX,dataModel.value);
